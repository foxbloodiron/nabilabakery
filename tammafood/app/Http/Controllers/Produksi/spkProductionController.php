...
$spk=d_spk::
           ...
           ->join('d_stock',function($join){
                ...
                $join->on('s_position','=',DB::raw('2'));                
                $join->orOn('s_item','=','i_id');
                $join->on('s_comp','=',DB::raw('3'));
                $join->on('s_position','=',DB::raw('3'));
                
           })->select('spk_date','i_name','pp_qty',DB::raw('sum(s_qty) as s_qty'))
             ->groupBy('spk_item','spk_date')
             ->get();              
            */
        $spk=d_spk::
             join('m_item','spk_item','=','i_id')
           ...
           ->select('spk_date','i_name','pp_qty','spk_code')
             ...
             ->paginate(1);              
        $productresult_dt=DB::
                          table('d_productresult')->join('d_productresult_dt','prdt_productresult','=','pr_id')->get();
        return view('/produksi/spk/spk',compact('productplan','spk','productresult_dt'));
    }
